...
router.get('/api/config', async (req, res) => { sendJson(res, 200, CONFIG); });

router.post('/api/auth/register', async (req, res) => {
  const { name, phone, email, password } = req._body;
  if (!name || !phone || !password) return sendJson(res, 400, { error: 'name, phone, and password are required' });
  const data = db.load();
  if (data.users.find((u) => u.phone === phone)) return sendJson(res, 409, { error: 'An account with this phone number already exists' });
  const user = { id: db.nextId(data, 'users'), name, phone, email: email || null, passwordHash: hashPassword(password), createdAt: new Date().toISOString() };
...
router.post('/api/auth/login', async (req, res) => {
  ...
  if (!user || !verifyPassword(password, user.passwordHash)) return sendJson(res, 401, { error: 'Invalid phone number or password' });
  const token = signToken({ sub: user.id, role: 'customer' });
...
router.post('/api/admin/bootstrap', async (req, res) => {
  ...
  data.admins.push({ id: 1, username, passwordHash: hashPassword(password) });
  db.save(data);
...
router.post('/api/admin/login', async (req, res) => {
  ...
  if (!admin || !verifyPassword(password, admin.passwordHash)) return sendJson(res, 401, { error: 'Invalid username or password' });
...
router.post('/api/bookings', async (req, res) => {
  ...
  if (!vehicleType || !serviceType || !address) return sendJson(res, 400, { error: 'vehicleType, serviceType, and address are required' });
  const data = db.load();
  const booking = { id: db.nextId(data, 'bookings'), userId: auth.sub, vehicleType, serviceType, address, lat: lat ?? null, lng: lng ?? null, scheduledTime: scheduledTime || null, status: 'pending', staffId: null, price: null, paymentStatus: 'unpaid', startLocation: null, endLocation: null, createdAt: new Date().toISOString() };
...
router.get('/api/bookings', async (req, res) => {
  ...
  if (!auth || auth.role !== 'customer') return sendJson(res, 401, { error: 'Login required' });
  const data = db.load();
  sendJson(res, 200, data.bookings.filter((b) => b.userId === auth.sub));
});
...
router.post('/api/bookings/:id/cancel', async (req, res) => {
  ...
  if (!booking) return sendJson(res, 404, { error: 'Booking not found' });
  if (['completed', 'cancelled'].includes(booking.status)) return sendJson(res, 400, { error: `Cannot cancel a ${booking.status} booking` });
...
router.post('/api/payments/initiate', async (req, res) => {
  ...
  if (!booking) return sendJson(res, 404, { error: 'Booking not found' });
  const payment = { id: db.nextId(data, 'payments'), bookingId, userId: auth.sub, amount: booking.price || 0, provider: 'phonepe', status: 'initiated', redirectUrl: `https://washwale.example/pay/mock/${randomToken(8)}`, createdAt: new Date().toISOString() };
...
router.get('/api/admin/bookings', async (req, res) => { if (!requireAdmin(req, res)) return; sendJson(res, 200, db.load().bookings); });
...
router.get('/api/admin/staff', async (req, res) => { if (!requireAdmin(req, res)) return; sendJson(res, 200, db.load().staff); });
...
router.post('/api/admin/staff', async (req, res) => {
  ...
  const data = db.load();
  const staff = { id: db.nextId(data, 'staff'), name, phone, trackLinkToken: randomToken(16), currentLocation: null, createdAt: new Date().toISOString() };
...
server.listen(PORT, () => { console.log(`WashWale API listening on port ${PORT}`); });
